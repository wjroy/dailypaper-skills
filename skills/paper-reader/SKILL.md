...
description: |
  ...
  **v2 pipeline integration**: 当从 daily-papers 流水线的 notes 阶段调用时，接收
...
### 1a. v2 pipeline 输入（从 daily-papers 流水线调用时）
...
1. **完整性**: 论文中的 Figure、公式、Table 应尽量全部出现在笔记中；无法提取的必须在"缺失字段报告"中如实记录
...
- [ ] 所有 Figure 都尽量包含在笔记中？缺失的已记录在缺失字段报告？
- [ ] 所有公式都在笔记中（变量一致、无冲突）？
- [ ] 所有 Table 完整保留（所有行列）？缺失的已记录？
- [ ] 正文中技术术语有 `[[概念]]` 内联链接？
- [ ] 概念库已更新（缺失的概念已创建）？
- [ ] 图片可用（外链可加载 / 本地 >10KB）？
- [ ] 缺失字段报告已填写（无遗漏无虚构）？
